...
spark = spark_build(
		{'spark.master': 'yarn'},
		app_name = 'test_dm_station_gobike_tripdata')
...
# загрузка измерения станций ods -> dm
...
from (
    -- отбираем станции начала поездок
    select 
      start_station_id as station_id,
      start_station_name as station_name,
      start_station_latitude as station_latitude,
      start_station_longitude as station_longitude,
      row_number() over (partition by start_station_id order by start_time desc) as rn
    from {0}
    union all
    -- отбираем станции конца поездок
    select 
...
-- оставляем актуальные версии
where rn = 1
""".format(ods_datatrip_table, etl_session_id))

num_records = dim_df.count()

dim_df.repartition(1)\
    .write.format("orc")\
    .mode('overwrite')\
    .saveAsTable(dm_station_table)
...
select 
  ...
  '{3}' as file_name,
   {4} as num_records
""".format(etl_session_table, etl_session_id, dm_station_table, '', num_records))
